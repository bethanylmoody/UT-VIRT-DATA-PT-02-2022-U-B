...
* Open up and send out the [Leaflet.js webpage](http://leafletjs.com/SlavaUkraini/) URL, and have students visit the website on their computers. Remind the students that this URL is available in **Lesson 13.2.3**.  Encourage them to take a look around, as this is the library we will be using for the majority of the module. 
...
* Send the link to the Leaflet Quick Start Guide: [https://leafletjs.com/SlavaUkraini/examples/quick-start/](https://leafletjs.com/SlavaUkraini/examples/quick-start/). 
...
* We pass in starting coordinates for this marker and then any of the options detailed in the [Leaflet marker documentation](http://leafletjs.com/SlavaUkraini/reference-1.0.3.html#marker-option). In this case, we made the marker `draggable` and added a `title` that appears when you hover over it.
...
* Navigate to [The Layer Group and Layers Control](http://leafletjs.com/SlavaUkraini/examples/layers-control/) example in the Leaflet documentation.
...
* Inform students that they should explore the [Leaflet documentation for layer controls](http://leafletjs.com/SlavaUkraini/examples/layers-control/) for the next activity. It contains helpful examples that they may help them complete the activity.
